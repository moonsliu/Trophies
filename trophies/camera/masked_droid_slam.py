...
DROID_ROOT = PROJECT_ROOT / "third_party" / "droid_slam"
...
def run_metric_slam(img_folder, masks=None, calib=None, is_static=False):
    ...
    # Recover metric depth with ZoeDepth.
    ...
    # Estimate metric translation scale.
    scales_ = []
    n = len(tstamp)
    ...
    # Convert to metric camera extrinsics: R_wc, T_wc
...
def calibrate_intrinsics(img_folder, masks=None, stride=10, max_frame=50,
                        ...
                        focal=None):
    """Estimate intrinsics, or validate a caller-provided focal length."""
    if masks is not None:
        masks = masks[::stride]
        masks = masks[:max_frame]
        img_msks, conf_msks = preprocess_masks(img_folder, masks)
    ...
    calib = np.array(est_calib(img_folder), dtype=np.float64)
    if focal is not None:
        focal = float(focal)
        if not np.isfinite(focal) or focal <= 0:
            raise ValueError(f"focal must be a finite positive value, got {focal}")
        calib[:2] = focal

        if is_static:
            return calib, True

        _, detected_static = test_slam(
            img_folder, input_msks, stride=stride, calib=calib, max_frame=max_frame
        )
        if detected_static:
            print('The camera is likely static.')
        print(f'Using caller-provided focal length: {focal:g} px')
        return calib, detected_static

    # User indicates a static camera without a focal override.
    if is_static:
        return calib, True

    # Estimate camera motion and search intrinsics.
    else:
        best_focal = calib[0]
        ...
        for candidate_focal in range(low, high, step):
            calib[:2] = candidate_focal
